  ...
  const submit = async (asDraft = false) => {
    ...
    if (!form.serviceId) {
      try {
        ...
        const match = services.find((s: any) => s.name.includes("حراسة") || s.category?.includes("أمن"));
        if (match) handleChange("serviceId", match.id);
      } catch {}
    }
    if (!form.serviceId) {
